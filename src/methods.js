...
export function getInstrumentByInstrumentId(instrumentId) {
    return InstrumentConstants.find( instrument => instrument.instrumentId === instrumentId);
}
 ...
 * @param {number} tuningId 
 */
export function getStringCountByInstrumentIdAndTuningId(instrumentId, tuningId) {
  const instrument = InstrumentConstants.find( instrument => instrument.instrumentId === instrumentId);
  const tuning = instrument.tunings.find( tuning => tuning.tuningId === tuningId);
 ...
 */
export function getFretNumberByInstrumentIdTuningIdStringNumberAndNoteName(instrumentId, tuningId, stringNumber, noteName) {
  const instrument = InstrumentConstants.find( instrument => instrument.instrumentId === instrumentId );
  const tuning = instrument.tunings.find( tuning => tuning.tuningId === tuningId);
  ...
  return ((noteNameNameIndex - stringNameIndex) < 1) ? (noteNameNameIndex - stringNameIndex) + 12 : (noteNameNameIndex - stringNameIndex);
}
 ...
 */
export function getNoteNameByInstrumentIdTuningIdStringNumberAndFretNumber(instrumentId, tuningId, stringNumber, fretNumber) {
  const tuning = getTuningByInstrumentIdAndTuningId(instrumentId, tuningId);
  const stringName = tuning.stringNames[stringNumber].toUpperCase();

  // Where the string name falls in the chromatic scale
  const stringNameIndex = Notes.indexOf(stringName);
  const newNoteIndex = stringNameIndex + fretNumber > 11 ? stringNameIndex + fretNumber -12 : stringNameIndex + fretNumber;
